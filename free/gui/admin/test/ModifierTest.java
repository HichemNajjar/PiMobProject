...
public class ModifierTest extends Form {
    ...
    public ModifierTest(Form previous) {
        super("Modifier", new BoxLayout(BoxLayout.Y_AXIS));
        this.previous = previous;

        currentTest = AfficherToutTest.currentTest;
    ...
    private void addGUIs() {
        ...
        titreTF.setText(currentTest.getTitre());
        categorieTF.setText(currentTest.getCategorie());
        descriptionTF.setText(currentTest.getDescription());
        prixTF.setText(String.valueOf(currentTest.getPrix()));

        certifPC.getPicker().setSelectedString(currentTest.getCertif().getNom());
        selectedCertif = currentTest.getCertif();
    ...
    private void addActions() {
        ...
        manageButton.addActionListener(action -> {
            if (controleDeSaisie()) {
                int responseCode = TestService.getInstance().edit(
                        new Test(
                                currentTest.getId(),
                ...
                if (responseCode == 200) {
                    Dialog.show("Succés", "Test modifié avec succes", new Command("Ok"));
                    showBackAndRefresh();
                } else {
                    Dialog.show("Erreur", "Erreur de modification de test. Code d'erreur : " + responseCode, new Command("Ok"));
                }
            }
        });
    }
